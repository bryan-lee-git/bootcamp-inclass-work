var unsortedArr = [
  ...
  //pass the array, the smallest value (left) and the largest value (right) into the function
  function quickSort(arr, left, right){
    //variable for human readable length of array
    var len = arr.length, 
    // pivot is the number you are checking against (the last item in the array)
    pivot,
    //partition index is the index where you will swap any values smaller than the pivot
    partitionIndex;
 
    // if the left value is smaller than the right value...
   if(left < right){
    // the pivot value is the largest number in the array (the right side)
     pivot = right;
     // partitionIndex runs the partition function using the values filled in from running the quickSort function and becomes that value
